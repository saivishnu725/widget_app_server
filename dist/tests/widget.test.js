...
describe('Widget Routes', () => {
    ...
    beforeAll(async () => {
        // Register owner
        const ownerRes = await (0, supertest_1.default)(app_1.default).post('/auth/register').send(owner);
        ownerToken = ownerRes.body.token;
        ownerId = ownerRes.body.user.id;
        // Register other user
        const otherRes = await (0, supertest_1.default)(app_1.default).post('/auth/register').send(otherUser);
        otherUserToken = otherRes.body.token;
        otherUserId = otherRes.body.user.id;
    });
    afterAll(async () => {
        // Clean up widgets
        await db_1.default.widget.deleteMany({
    ...
    describe('POST /api/widgets', () => {
        it('should create a new widget successfully', async () => {
            ...
            const redisState = await redis_1.default.get(`widget:${testWidgetId}:state`);
